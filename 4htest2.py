...
class TestStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if(((rsi_k_current - rsi_k_trailing > 3) or (price_current < lossStop)) and currentPosition.inPosition == True):
            self.log('SELL')
            # Win
            if(rsi_k_current < rsi_k_trailing):
                positionStats.wins += 1
                feeWin = ((((positionStats.capital * leverage))/100)*0.06) * 2
                positionStats.capital = positionStats.capital + (((((positionStats.capital * leverage)/currentPosition.buyPrice)*(price_current)) - (positionStats.capital * leverage)) - feeWin)
